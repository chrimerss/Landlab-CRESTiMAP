    ...
    **Additional References**
    ...
    def __init__(
        ...
        **callback_kwds,
        ...
        self._is_cores= (self._grid.status_at_node!=self._grid.BC_NODE_IS_CLOSED)
        ...
        self._zgw = self._grid.at_node["ground_water__elevation"]
        self._zgw[self._grid.status_at_node==self._grid.BC_NODE_IS_CLOSED]= 0

        self._thickness = self._grid.at_node["aquifer__thickness"]
        self._thickness[:] = self._zgw - self._base
        self._thickness[grid.closed_boundary_nodes] = 0

        self._hydr_grad = self._grid.add_zeros('hydraulic__gradient',
                                             at='link',clobber=True)
        self._base_grad = self._grid.add_zeros("aquifer_base__gradient", at="link",clobber=True)

        self._qgw = self._grid.at_link["groundwater__specific_discharge"]
        self._qs = self._grid.at_node["surface_water__discharge"]
        self._qsavg = self._grid.add_zeros("average_surface_water__specific_discharge",
                         at='node',clobber=True)

        self._vel = self._grid.add_zeros("groundwater__velocity",at='link',clobber=True)

        self._dhdt = self._grid.add_zeros("water_table__velocity",at='node',clobber=True)
    ...
    def run_one_step(self, dt):
        ...
        if (self._zgw[self._cores] > self._elev[self._cores]).any():
            warn(
                "water table above elevation surface. "
                "Setting water table elevation here to "
                "elevation surface"
            )
            self._zgw[self._zgw > self._elev] = self._elev[
                self._zgw > self._elev
            ]
            self._zsf[self._zgw > self._elev]= self._zgw[self._zgw > self._elev]
            self._thickness[self._cores] = (
                self._zgw[self._cores] - self._base[self._cores]
            )
        ...
        self._hydr_grad[self._grid.active_links] = (
            self._grid.calc_grad_at_link(self._zgw)[self._grid.active_links]
        ...
        hlink = (
            map_value_at_max_node_to_link(
                self._grid, "ground_water__elevation", "aquifer__thickness"
            )
            * cosa
        )

        # Calculate specific discharge
        self._qgw[:] = hlink * self._vel

        # Groundwater flux divergence
        dqdx = self._grid.calc_flux_div_at_node(self._qgw)
        ...
        self._qs[:] = _regularize_G(rel_thickness, self._r) * _regularize_R(
            self._recharge - dqdx
        )*self._grid.dx*self.grid.dy

        # Mass balance
        self._dhdt[:] = (1 / self._n) * (self._recharge - self._qs/self._grid.dx/self.grid.dy - dqdx)
        ...
        self._zgw[self._cores] = (self._base + self._thickness)[self._cores]
    ...
    def run_with_adaptive_time_step_solver(self, dt):
        ...
        if (self._zgw[self._cores] > self._elev[self._cores]).any():
            ...
            # print(self._zgw[self._zgw > self._elev], self._elev[self._zgw > self._elev])
            self._zsf[(self._zgw > self._zsf)&(self._is_cores)]= self._zgw[(self._zgw > self._zsf) & (self._is_cores)]
            self._zgw[(self._zgw > self._elev)&(self._is_cores)] = self._elev[
                (self._zgw > self._elev) &(self._is_cores)
            ]
            self._thickness[self._cores] = (
                self._zgw[self._cores] - self._base[self._cores]
            )
        ...
        while remaining_time > 0.0:
            ...
            hlink[hlink<1e-3]=1e-3 #to prevent numerical explosion
            ...
            self._dt_vn = self._vn_coefficient * np.nanmin(
                self._n_link[self._grid.active_links] * self._grid.length_of_link[self._grid.active_links] ** 2 /\
                 abs(4 * self._K[self._grid.active_links] *\
                 hlink[self._grid.active_links])
            )
            np.seterr(divide='ignore')
            self._dt_courant = self._courant_coefficient * np.nanmin(
                self._grid.length_of_link[self._grid.active_links] /\
                 abs(self._vel[self._grid.active_links] /\
                  self._n_link[self._grid.active_links])
            )
            dt_stability = min(self._dt_courant, self._dt_vn)
            substep_dt = min([dt_stability, remaining_time])
            # print('grounwater dt: ',substep_dt)
            # Update
        ...
        self._grid.at_link["surface_water__discharge"] += self._grid.map_max_of_link_nodes_to_link(self._qsavg)
